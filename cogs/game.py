			...
			if str(reaction.emoji) == '<:myright:584973342793138197>':
				...
				if message:
					...
					try:
						...
						if str(reaction.emoji) == '<:myA:584978563204120577>':
							await message.edit(content="**Hurray!!! I made it.** <a:tsunglasses:594132185561300992>")

						elif str(reaction.emoji) == '<:myB:592285509863211009>':
							await message.edit(content="**I might try my best next time. Thanks for playing** <a:tcryingface:581729105230168068>")
		...
		if player is not None:
			...
			if not player.bot:
				...
				def check_both_player(reaction, user):
					return (user == ctx.message.author or user == player) and str(reaction.emoji) == '<:treplay:593851739170668595>' and (reaction.message.id == message.id)
				...
				embed.set_footer(text=f"Currently Playing | {player1.display_name} vs {player2.display_name}", icon_url="https://raw.githubusercontent.com/J16N/tsuby/master/cogs/tsuby.png")
				...
				while True:
					...
					if not initial_move % 2 and not initial_move == 9:
						...
						try:
							...
							if not reaction.emoji.name == "tquit":
								...
								await message.clear_reactions()
								await message.edit(content=f"*Oh no, {player1.display_name} left the game.* <a:tcryingface:581729105230168068>", embed=None)
								break


						except asyncio.TimeoutError:
							await message.clear_reactions()
							await message.edit(content=f"***{player1.mention} missed the turn. What a noob!*** <a:tlol:594138185668362241>", embed=None)
							break
						...
						try:
							...
							if not reaction.emoji.name == "tquit":
								...
								if self.check_win(winning_string):
									await message.clear_reactions()
									await message.edit(content=f"**Congratulation !!**\n{player2.mention} won. <a:trainbowconfetti:572049858643623956>\n*In order to play again, react both of you.*", embed=None)
								...
								await message.clear_reactions()
								await message.edit(content=f"*Oh no, {player2.display_name} left the game.* <a:tcryingface:581729105230168068>", embed=None)
								break


						except asyncio.TimeoutError:
							await message.clear_reactions()
							await message.edit(content=f"***{player2.mention} missed the turn. What a noob!*** <a:tlol:594138185668362241>", embed=None)
							break
			...
			if ctx.author.id == 302467968095223820: #this feature currently needs to be tested
